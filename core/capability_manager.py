...
class CapabilityManager:
    ...
    def report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "reference_database": {"available": bool(self.config.get("_reference", {}).get("available")), "reason": None if self.config.get("_reference", {}).get("available") else self.config.get("_reference", {}).get("reason", "No reference database configured"), "records": self.config.get("_reference", {}).get("records", 0)},
