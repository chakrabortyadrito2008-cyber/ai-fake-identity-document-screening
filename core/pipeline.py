...
from modules.checksum_validator import ChecksumValidator
...
class FraudPipeline:
    ...
    def screen(self, file_path: str | Path, identity_key: str | None=None, request_id: str | None=None, reference_face_path: str | Path | None=None, live_selfie_path: str | Path | None=None) -> dict[str,Any]:
        ...
        blocked_by_quality={"document_type","document_layout","ocr_engine","identity_extractor","field_confidence","document_validator","checksum_validator","qr_verification","security_features","forensics","face_detection","aadhaar_template","face_quality","face_document_match","liveness_detection","presentation_attack","deepfake_detection","anomaly_detection"}
        ...
        return result
    def screen_folder(self, folder_path: str | Path, identity_key_prefix: str | None=None, limit: int | None = None) -> dict[str,Any]:
        ...
        for index,path in enumerate(files):
            ...
            try: results.append(self.screen(path,key,request_id=f"{batch_id}:{index}"))
        ...
        report={"batch_id":batch_id,"created_at":datetime.now(timezone.utc).isoformat(),"source_folder":str(folder),"file_count":len(files),"triage_counts":counts,"results":results}
