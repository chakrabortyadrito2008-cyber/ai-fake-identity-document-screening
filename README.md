...
The operational labels are `LIKELY GENUINE — NOT VERIFIED`, `NEEDS MANUAL VERIFICATION`, and `LIKELY FAKE / FRAUD SIGNALS`. `LIKELY GENUINE` requires both low risk and a matching non-demo approved trusted source; low risk alone is escalated to manual verification. Any high-confidence fraud or presentation-attack signal is also escalated. These remain human-review triage labels, not a legal declaration that a document is genuine or fake.
