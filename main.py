...
def main():
    ...
    sub=p.add_subparsers(dest='command',required=True); s=sub.add_parser('screen');s.add_argument('file');s.add_argument('--identity-key');s.add_argument('--reference-face');s.add_argument('--live-selfie',help='Separate newly captured selfie image for PAD/liveness analysis'); f=sub.add_parser('screen-folder',help='Screen all supported document images in one folder');f.add_argument('folder');f.add_argument('--identity-key-prefix');f.add_argument('--limit',type=int,help='Process only the first N supported files'); sub.add_parser('self-test'); a=sub.add_parser('serve');a.add_argument('--host',default='127.0.0.1');a.add_argument('--port',type=int,default=8000); r=sub.add_parser('purge-retention');r.add_argument('--days',type=int)
    args=p.parse_args(); pipe=FraudPipeline(ROOT)
    if args.command=='screen':print(json.dumps(pipe.screen(args.file,args.identity_key,reference_face_path=args.reference_face,live_selfie_path=args.live_selfie),indent=2))
    elif args.command=='screen-folder': print(json.dumps(pipe.screen_folder(args.folder,args.identity_key_prefix,limit=args.limit),indent=2))
